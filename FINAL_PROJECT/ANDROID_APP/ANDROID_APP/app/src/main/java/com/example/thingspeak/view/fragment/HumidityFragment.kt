...
class HumidityFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lineChart = view.findViewById(R.id.lineChartHumi) // Kết nối LineChart từ giao diện

        // Quan sát dữ liệu từ ViewModel
        viewModel.data.observe(viewLifecycleOwner) { dataList ->
            updateChart(dataList) // Cập nhật biểu đồ mỗi khi dữ liệu thay đổi0
        }

        // Cài đặt Runnable để tự động cập nhật dữ liệu sau mỗi 10 giây
        updateRunnable = kotlinx.coroutines.Runnable {
            viewModel.fetchAllFields(
                "MI60P3HS5SH5CEWE",
                results = 100
    ...
    private fun updateChart(dataList: List<ThingSpeakData>) {
        ...
        val entries = dataList.mapIndexed { index, data ->
            ...
            val yValue = data.field2?.toFloat() ?: 0f
            Entry(xValue, yValue)
        }
